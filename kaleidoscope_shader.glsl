// This is the shader that W-M wrote, that is hosted on Shadertoy at https://www.shadertoy.com/view/ldsXWn
// it might be adaptable to Unity.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	uv *= ZOOM_FACTOR+(BASS_INTENSITY)* (texture2D(iChannel1, vec2(.01,0)).r);
	
	vec3 final_color = vec3( 0.0 );
	float final_density = 0.0;
	for ( int i = 0; i < 128; i++ ) {
		vec4 noise  = texture2D( iChannel0, vec2( float( i ) + 0.5, 0.5 ) / 256.0 );
		vec4 noise2 = texture2D( iChannel0, vec2( float( i ) + 0.5, 9.5 ) / 256.0 );
		
		
		//sound loudness to intensity
		float velintensity = texture2D(iChannel1, vec2(1.0+uv.x,0)).r;
		...
		//sound loudness to intensity
		float intensity = texture2D(iChannel1, vec2(pos.y,0)).r;
